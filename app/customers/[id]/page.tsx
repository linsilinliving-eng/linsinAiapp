...
// ─── CSS ──────────────────────────────────────────────────────────────────────
const CSS = `
.cdt-root{min-height:100vh;background:#f3f4f8;font-family:'Sarabun',sans-serif;font-size:15px;padding:28px 32px 80px}
.cdt-root *{box-sizing:border-box}
.cdt-back{
  display:inline-flex;align-items:center;gap:6px;color:#3C3489;
  font-size:13px;font-weight:600;cursor:pointer;margin-bottom:16px;
  background:none;border:none;padding:0;font-family:inherit;
}
.cdt-back:hover{text-decoration:underline}
.cdt-hero{
  background:linear-gradient(135deg,#3C3489,#5549b0);
...
.cdt-hero .sub{font-size:13px;opacity:.8}
...
.cdt-tab:hover{background:#EEEDFE;color:#3C3489;border-color:#c4bff8}
.cdt-tab.active{background:#3C3489;color:#fff;border-color:#3C3489}
.cdt-panel{background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:24px}
.cdt-section-title{font-weight:700;font-size:15px;color:#3C3489;margin-bottom:16px;display:flex;align-items:center;gap:8px}
.cdt-field-row{display:grid;grid-template-columns:160px 1fr;gap:8px;padding:8px 0;border-bottom:1px solid #f3f4f6;align-items:start}
.cdt-field-row:last-child{border-bottom:none}
.cdt-field-lbl{font-size:13px;color:#6b7280;padding-top:2px}
.cdt-field-val{font-size:14px;color:#1f2937;font-weight:500}
.cdt-input{width:100%;border:1px solid #d1d5db;border-radius:7px;padding:8px 12px;font-size:14px;font-family:inherit;outline:none;transition:border .15s}
.cdt-input:focus{border-color:#3C3489;box-shadow:0 0 0 3px rgba(60,52,137,.1)}
.cdt-select{width:100%;border:1px solid #d1d5db;border-radius:7px;padding:8px 12px;font-size:14px;font-family:inherit;background:#fff}
.cdt-row2{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.cdt-row3{display:grid;grid-template-columns:2fr 1fr 1fr;gap:12px}
.cdt-lbl{display:block;font-size:12px;font-weight:600;color:#374151;margin-bottom:4px}
.cdt-addr-card{background:#f8f9ff;border:1px solid #c4bff8;border-radius:8px;padding:14px;margin-bottom:10px}
.cdt-contact-card{background:#f8f9ff;border:1px solid #c4bff8;border-radius:8px;padding:14px;margin-bottom:10px}
.cdt-checks{display:flex;flex-wrap:wrap;gap:8px;margin-top:6px}
.cdt-check-chip{
  display:flex;align-items:center;gap:6px;
  background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;
  padding:5px 12px;cursor:pointer;font-size:13px;transition:all .15s;
}
.cdt-check-chip.selected{background:#EEEDFE;border-color:#c4bff8;color:#3C3489;font-weight:600}
.cdt-flag-checks{display:flex;gap:12px;flex-wrap:wrap}
.cdt-flag-check{display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer}
.cdt-btn{border:none;border-radius:8px;padding:8px 18px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit}
.cdt-btn-primary{background:#3C3489;color:#fff}
.cdt-btn-primary:hover{background:#2d2670}
.cdt-btn-primary:disabled{opacity:.5;cursor:not-allowed}
.cdt-btn-danger{background:#c0392b;color:#fff}
.cdt-btn-danger:hover{background:#a93226}
.cdt-btn-sm{background:#fff;border:1px solid #e5e7eb;border-radius:6px;padding:5px 12px;font-size:12px;cursor:pointer;font-family:inherit;color:#374151}
.cdt-btn-sm:hover{background:#f3f4f6}
.cdt-btn-sm.danger:hover{background:#fdecea;color:#c0392b;border-color:#fca5a5}
.cdt-save-bar{display:flex;gap:10px;align-items:center;margin-top:20px;padding-top:16px;border-top:1px solid #f3f4f6}
.cdt-add-btn{
  border:1.5px dashed #c4bff8;background:transparent;color:#3C3489;
  border-radius:8px;padding:7px 14px;font-size:13px;cursor:pointer;
  display:flex;align-items:center;gap:6px;font-family:inherit;
}
.cdt-add-btn:hover{background:#EEEDFE}
...
.cdt-history-row{display:flex;gap:12px;padding:8px 0;border-bottom:1px solid #f3f4f6;font-size:13px}
...
.cdt-info{background:#EEEDFE;border-left:3px solid #3C3489;border-radius:0 6px 6px 0;padding:10px 14px;font-size:13px;color:#2d2670;margin-bottom:14px}
        ...
        <div className="cdt-hero">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
            <div>
              ...
              <div className="sub">{data.business_type || '—'} · {data.cus_type === 'company' ? 'นิติบุคคล' : 'บุคคลธรรมดา'}</div>
            </div>
          ...
          <div className="meta" style={{ fontSize: 13, opacity: .85, marginTop: 10 }}>
        ...
        {tab === 'info' && (
          <div className="cdt-panel">
            ...
            <div className="cdt-row2" style={{ marginBottom: 14 }}>
              <div>
                <label className="cdt-lbl">ชื่อ *</label>
                <input className="cdt-input" value={cusName} onChange={e => setCusName(e.target.value)} />
              </div>
              <div>
                <label className="cdt-lbl">ชื่อเล่น</label>
                <input className="cdt-input" value={nickname} onChange={e => setNickname(e.target.value)} />
              </div>
            </div>
            <div className="cdt-row2" style={{ marginBottom: 14 }}>
              <div>
                <label className="cdt-lbl">เลขประจำตัวผู้เสียภาษี</label>
                <input className="cdt-input" value={taxId} onChange={e => setTaxId(e.target.value)} maxLength={13} />
              </div>
              <div>
            ...
            {data.cus_type === 'company' && (
              <div className="cdt-row2" style={{ marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <input className="cdt-input" value={branchNo} onChange={e => setBranchNo(e.target.value)} disabled={branchType === '-'} />
                </div>
              </div>
            )}
            <div className="cdt-row2" style={{ marginBottom: 14 }}>
              <div>
                <label className="cdt-lbl">จำนวนวันเครดิต</label>
                <input className="cdt-input" type="number" value={creditDay} onChange={e => setCreditDay(e.target.value)} />
              </div>
              <div>
                <label className="cdt-lbl">สถานะ</label>
            ...
            <div style={{ marginBottom: 14 }}>
              <label className="cdt-lbl">หมายเหตุ</label>
              <textarea className="cdt-input" rows={3} value={remark} onChange={e => setRemark(e.target.value)} />
            </div>
          ...
          <div className="cdt-panel">
                ...
                {editingAddr === a.id ? (
                  <>
                    <div className="cdt-row2" style={{ marginBottom: 10 }}>
                      <div>
                        <label className="cdt-lbl">Label</label>
                        <input className="cdt-input" value={addrForm.label || ''} onChange={e => setAddrForm(p => ({ ...p, label: e.target.value }))} />
                      </div>
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <label className="cdt-lbl">บรรทัด 1 *</label>
                      <input className="cdt-input" value={addrForm.address_line1 || ''} onChange={e => setAddrForm(p => ({ ...p, address_line1: e.target.value }))} />
                    </div>
                    <div className="cdt-row3" style={{ marginBottom: 10 }}>
                      <div><label className="cdt-lbl">แขวง/ตำบล</label><input className="cdt-input" value={addrForm.sub_district || ''} onChange={e => setAddrForm(p => ({ ...p, sub_district: e.target.value }))} /></div>
                      <div><label className="cdt-lbl">เขต/อำเภอ</label><input className="cdt-input" value={addrForm.district || ''} onChange={e => setAddrForm(p => ({ ...p, district: e.target.value }))} /></div>
                      <div><label className="cdt-lbl">จังหวัด</label><input className="cdt-input" value={addrForm.province || ''} onChange={e => setAddrForm(p => ({ ...p, province: e.target.value }))} /></div>
                    </div>
                    <div style={{ marginBottom: 10 }}>
                      <label className="cdt-lbl">รหัสไปรษณีย์</label>
                      <input className="cdt-input" value={addrForm.postal_code || ''} onChange={e => setAddrForm(p => ({ ...p, postal_code: e.target.value }))} style={{ width: 100 }} maxLength={5} />
                    </div>
            ...
            {newAddr ? (
              <div className="cdt-addr-card">
                <div style={{ fontWeight: 700, fontSize: 14, color: '#3C3489', marginBottom: 12 }}>➕ ที่อยู่ใหม่</div>
                <div className="cdt-row2" style={{ marginBottom: 10 }}>
                  <div><label className="cdt-lbl">Label</label><input className="cdt-input" value={addrForm.label || 'บริษัท'} onChange={e => setAddrForm(p => ({ ...p, label: e.target.value }))} /></div>
                  <div style={{ display: 'flex', alignItems: 'flex-end' }}><label className="cdt-flag-check"><input type="checkbox" checked={!!addrForm.is_default} onChange={e => setAddrForm(p => ({ ...p, is_default: e.target.checked }))} />⭐ หลัก</label></div>
                </div>
                <div style={{ marginBottom: 10 }}><label className="cdt-lbl">บรรทัด 1 *</label><input className="cdt-input" value={addrForm.address_line1 || ''} onChange={e => setAddrForm(p => ({ ...p, address_line1: e.target.value }))} /></div>
                <div className="cdt-row3" style={{ marginBottom: 10 }}>
                  <div><label className="cdt-lbl">แขวง/ตำบล</label><input className="cdt-input" value={addrForm.sub_district || ''} onChange={e => setAddrForm(p => ({ ...p, sub_district: e.target.value }))} /></div>
                  <div><label className="cdt-lbl">เขต/อำเภอ</label><input className="cdt-input" value={addrForm.district || ''} onChange={e => setAddrForm(p => ({ ...p, district: e.target.value }))} /></div>
                  <div><label className="cdt-lbl">จังหวัด</label><input className="cdt-input" value={addrForm.province || ''} onChange={e => setAddrForm(p => ({ ...p, province: e.target.value }))} /></div>
              ...
              <button className="cdt-add-btn" onClick={() => { setNewAddr(true); setAddrForm({ label: 'บริษัท', use_for_invoice: true, use_for_shipping: true, use_for_install: false }); }}>
                + เพิ่มที่อยู่อีก
              </button>
            )}
          </div>
          ...
          <div className="cdt-panel">
            ...
            {contacts.map(c => (
              <div key={c.id} className="cdt-contact-card">
                {editingContact === c.id ? (
                  <>
                    <div className="cdt-row2" style={{ marginBottom: 10 }}>
                      <div><label className="cdt-lbl">ชื่อ-นามสกุล *</label><input className="cdt-input" value={contactForm.full_name || ''} onChange={e => setContactForm(p => ({ ...p, full_name: e.target.value }))} /></div>
                      <div><label className="cdt-lbl">ชื่อเล่น</label><input className="cdt-input" value={contactForm.nickname || ''} onChange={e => setContactForm(p => ({ ...p, nickname: e.target.value }))} /></div>
                    </div>
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div className="cdt-checks">
                        {CONTACT_ROLES.map(role => (
                          <label
                            key={role}
                            className={`cdt-check-chip ${(contactForm.roles_arr || []).includes(role) ? 'selected' : ''}`}
                            onClick={() => {
                    ...
                    <div className="cdt-row2" style={{ marginBottom: 10 }}>
                      <div><label className="cdt-lbl">เบอร์โทร 1</label><input className="cdt-input" value={contactForm.phone1 || ''} onChange={e => setContactForm(p => ({ ...p, phone1: e.target.value }))} /></div>
                      <div><label className="cdt-lbl">เบอร์โทร 2</label><input className="cdt-input" value={contactForm.phone2 || ''} onChange={e => setContactForm(p => ({ ...p, phone2: e.target.value }))} /></div>
                    </div>
                    <div style={{ marginBottom: 10 }}><label className="cdt-lbl">อีเมล</label><input className="cdt-input" type="email" value={contactForm.email || ''} onChange={e => setContactForm(p => ({ ...p, email: e.target.value }))} /></div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      ...
                      {parseRoles(c.roles).length > 0 && (
                        <div style={{ fontSize: 12, color: '#3C3489', marginTop: 2 }}>{parseRoles(c.roles).join(' · ')}</div>
                      )}
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button className="cdt-btn-sm" onClick={() => {
                        ...
                        setContactForm({ ...c, roles_arr: parseRoles(c.roles) });
                      }}>✏️</button>
            ...
            {newContact ? (
              <div className="cdt-contact-card">
                <div style={{ fontWeight: 700, fontSize: 14, color: '#3C3489', marginBottom: 12 }}>➕ ผู้ติดต่อใหม่</div>
                <div className="cdt-row2" style={{ marginBottom: 10 }}>
                  <div><label className="cdt-lbl">ชื่อ-นามสกุล *</label><input className="cdt-input" value={contactForm.full_name || ''} onChange={e => setContactForm(p => ({ ...p, full_name: e.target.value }))} /></div>
                  <div><label className="cdt-lbl">ชื่อเล่น</label><input className="cdt-input" value={contactForm.nickname || ''} onChange={e => setContactForm(p => ({ ...p, nickname: e.target.value }))} /></div>
                </div>
                <div style={{ marginBottom: 10 }}>
                  <label className="cdt-lbl">ตำแหน่ง</label>
                  <div className="cdt-checks">
                    {CONTACT_ROLES.map(role => (
                ...
                <div className="cdt-row2" style={{ marginBottom: 10 }}>
                  <div><label className="cdt-lbl">เบอร์โทร 1</label><input className="cdt-input" value={contactForm.phone1 || ''} onChange={e => setContactForm(p => ({ ...p, phone1: e.target.value }))} /></div>
                  <div><label className="cdt-lbl">อีเมล</label><input className="cdt-input" value={contactForm.email || ''} onChange={e => setContactForm(p => ({ ...p, email: e.target.value }))} /></div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="cdt-btn cdt-btn-primary" onClick={saveContact} disabled={saving}>{saving ? '⏳' : '💾 บันทึก'}</button>
